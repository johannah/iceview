...
****
TODO: incremental, orthorectified, low-altitude aerial images. 
Incemental mosaicking means we update the mosaic as new images are added to the 
collection meaning that the overview mosaic is produced quickly. The first method combines 
metatdata from the UAV such as GPS positions and UAV orientation to speed up the 
matching process. The second method does not utilize any external information 
and relies only on the images to produce the mosaic. 
****
...
the size. Operating an underwater vehicle is demanding TODO. With data about
and icebergs shape above water as gathered by an aerial platform, we can obtain information 
aboutthe size of the iceberg's sail. 
...
Digital cameras are convienent sensors for capturing and recording a specific 
...
with the human eye, but often other ranges of the spectrum 
are used in remote monitoring. 
...
In addition, segmentation and classification are two additional tasks that 
simplify the data into categories and allow for more complex data analysis. 


\section{Unmanned Aerial Vehicles}
UAVs have a wide variety of capabilities in terms of payload, endurance, weight, 
and speed and can vary widely in cost. One important consideration for shipboard 
...
\section{Sensors}
In addition to digital cameras, sea ice is also frequently monitored by 
active sensors which can 
build models of the physical geometry of the ice. Synthetic Aperture Radars (SARs) 
are a form of high-resolution active microwave that is commonly used to determine the 
age of ice and differentiate between ice and water. Radar can effectively evalutate 
sea ice thickness and allow estimates of ice density. Underwater acoustic sensors 
are used in the water to measure iceberg size and shape using sound. 
...
satellite data to provide a weekly macro observation every week 
weekly using Synthetic Aperture Radar imagery.
image mosaics produced weekly for the Canadian Artic
They also produce Daily Ice charts, which are an estimate of the current ice 
...
attempting to maintain Dynamic Positioning (DP) in ice-laden waters \cite{Skjetne}.
...
Edge-preserving MRF to segment sea ice \cite{Yang09}

Phd thesis \cite{Haugen14}
...
\cite{Milgram75, Milgram77, Szeliski97, Shum00, Marcon13, Brown03, Brown07 },
...
An image mosaic is an image which is built from a set of smaller images with 
an overlapping field of view. Constructing an image mosaic consists of several general 
steps. 
...
Early work in digital mosaicking generally fell into two camps,
direct and feature based. Direct methods attempt to use correlations 
over the entire image to find overlapping regions in images. This involes 
a computation across all of the pixels in the image and can be
slow and memory intensive \cite{Lucas81, Barnea72, Szeliski95, Szeliski97, Shum-local98, Irani00}.  
Feature
based methods, on the other hand, attempt to only work with interesting regions 
of the image, such as lines or edges. These interesting regions, called \emph{features} 
or \emph{keypoints}, are extracted using a \emph{feature descriptor}. The descriptors 
from images are then compared resulting in a measure of similarity between the 
...
tranformations \cite{Tuytelaars08, Forstner86, Lowe99, Brown03, Hu06, Elibol08}. 
Our approach utilizes features and is discussed more thouroughly in 
Chapter \ref{chap:features}.
TODO: panoramic vs mosaic
\subsection{A Typical Mosaic Process}
\begin{enumerate}
    \item{\textbf{Determine images with overlapping views of a scene.} 
        A reasonable amount of scene overlap (about 15-30\% overlap) is typically 
       required to determine that two images can be stitched together. The images 
   may have a different view that distorts the scene by changes in illumination,
   motion, or a host of other changes. These changes add significant difficulty 
   to determining overlapping images. There are many methods of determining how 
   much if any overlap is present between two images. Most modern approaches 
   use feature descriptors that are engineered to be invariant to common changes 
   between views. Knowing the location of the viewpoint from 
   ...
   of the motion of the camera.}
    \item{\textbf{Estimate the homography between images with overlapping scenes.} 
            TODO: define homography. After the corresponding pixels 
            between overlapping images have been identified, we need to estimate
            the correct allignment that will relate the collections of images. This 
            process is called image registration and attempts to find an alignment 
            between overlapping images that minimizes the difference between overlapping 
            pixels.
            \ref{subsec:removeoutliers} 
       }
   \item{\textbf{Align overlapping images.} 
           The homography calculated in the previous step 
           is used to warp the image onto the reference plane. Alignment involves 
           changing the coordinate system of each image onto the mosaic coordinate 
           system. 
    }
\item{\textbf{Blend image seams in the final mosaic.}
    ...
    methods of performing blending. TODO get into this}
\end{enumerate}
\subsection{Defining the Transformation}
           The transformations 
           can be translation, similarity, affine, or projective as described 
           by the transformation matrix. TODO explain transformations 
           The projective transformation is the farthest an 
           image can be transformed. In the projective transform,  
           the only visible feature that is preserved is strait lines. It can 
           be described as $x'=H*x$ where $x$ is the image coordinate system and 
           $x'$ is the mosaic coordinate system. $H$ is the homography matrix 
           calculated in the previous step. An affine 
           transformation also preserves parallelism.  TODO more
...
\subsection{Removing Outliers}
            A common approach to this is known as RANSAC, or 
             "RANdom SAmple Consensus. RANSAC is a well known and widely used 
             iterative parameter 
            estimation technique. It works by taking random samples from a set 
            of data and 
            fitting it to a model. 

\subsection{Projection Models}
...
***
tranformation performed leads to distrotion in scale and relative distances, 
which is not ideal for our case in which we wish to presenta georeferenced mosaic. 
The use of a non-planar surface is one of the main reasons for distortion 
because feature points are selected from different elevation levels for image registration. 
***
Exploit known ground reference points to enable georeferencing \cite{Pesti08}
or working off 
of a known map with fixed infrastructure (usually a satellite image of the region) \cite{TODO}. 
Satellite imagery is 
too low of an update for use in the sea with constantly moving ice.
...
medium resolution 800x600 pixels 
 ...
 globally consistent mosaic uses all overlap information, including overlap from images that are not consecutive in time
...
thesis is to develop methods for mosaicking of individual aerial images taken from
homogeneous or heterogeneous sensors on small-scale UAVs. The mosaicking of images
taken in such scenarios are challenging as compared to panoramic construction
or other mosaicking methods such as satellite image mosaicking. When flying with
UAVs at a relatively low altitude (below 100m), non-planar objects on the ground
make the feature matching and image registration more di!cult.
...
***
For example, the edge-based region
detector requires curves, and if none of sufficient length
occur in a particular image, then no regions of this type
can be detected.
However, this variety is also a virtue: the different
detectors are complementary. Some respond well
to structured scenes (e.g. MSER and the edge-based
regions), others to more textured scenes (e.g. HarrisAffine
and Hessian-Affine)


Also the size of the detected regions
significantly varies depending on the detector. Typically,
Harris-Affine, Hessian-Affine and MSER detect
many very small regions

Larger regions typically have a
higher discriminative power, as they contain more information,
which makes them easier to match, at the
cost of a higher risk of being occluded or not covering
a planar part of the scene.
***
\label{chap:features}
...
****
If the scene is planar (which is, in general, a valid ap- proximation when the ratio between the distance to the scene and the ground elevation is high \cite{Pizarro03}, a set of matches between two views can be used to estimate a homographic model for the apparent image motion. This model can be composed with the estimated motion of the rest of images in order to build the mosaic.

Acompletehomographyhas8degreesoffreedom(asitis defineduptoascalefactor).Thedegreesoffreedomcanbe reducedbyfixingsomeoftheparametersofthe3x3matrix. ThemodelsusedarethedefinedbyHartleyin Euclidean, Affine and Complete Homographic models, which have 4, 6 and8degreesoffreedomrespectively
...
images with excessive overlap are not used in the final mosaic as their 
inclusion provides no benefit and requires unnecessary computation.
****
pre-rotate and scale based on a reference image
 By using the homography obtained from the
 first step, we roughly align the UAV image with the global
 map. The first step provides us an initialization which basically
 compensates for the
 \cite{Lin07}
 ***
...
****
 (Qian et al., 2000) demonstrates the use of vehicle rotation rate information from gyros mounted to the vehicle to assist in the process of matching camera features from frame to frame.
...
Features are selected to provide robustness in front of certain
degree of distortion, so that the same point can be detected
when observed from a different view point.

uses Zernike moments, which are robust
to scale and rotation. More robust approaches are SIFT [23]
and more recently SURF [24], which produce rotation and
scale invariant features. SIFT and SURF features are becoming
important features in recent approaches

\cite{Oceans11} Aulinas
distinguishes high quality features using :
The second stage is focused on detecting RoIs within
these images, i.e. segmenting RoIs. In order to do so, two
parallel segmentation processes are computed. Both of them
are based on image processing common operations. The first
process starts with edge detection, producing the binary image
shown in Fig. 6(b). Afterwards, erode/dilate operations are
conducted, joining regions and eliminating insignificant spots
(see Fig. 6(c)). Next step is a region search within this
black and white image, producing the segmentation shown in
Fig. 6(d). On the other hand, the second process uses the Hue
channel (see Fig. 6(e)). This channel is then blurred in order
to smooth the whole image. Afterwards, a threshold is applied,
giving the results shown in Fig. 6(f)). This threshold is selected
according to the mean value of the Hue image. Afterwards,
a region search is conducted, producing the results shown in
Fig. 6(g). At this point both processes are fused: a RoI is
selected through the intersection of both segmentations (see
Fig. 6(h)).
***
...
A Homography is a transformation a 3 by 3 matrix that maps the points in one image to
