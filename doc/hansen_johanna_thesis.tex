...
to replace manned helicopters acting as scouts for 
 ships navigating in ice laden waters. As one can imagine, manned helicopters 
...
Structure from motion (SFM) \cite{Koch98, Pollefeys04} use cameras to infer 
depth from the scene nd then reconstruct the terrain in 3D using
image pairs and esitimating pose of the camera. Don't scale to long sequences 
\cite{MonoSLAM}
and for full 3D 
reconstruction \cite{Morimoto97, Shum-3D98}. 

Simultaneous Localization and Mapping (SLAM) moving sensor platform constructing 
a representation of its environment whil also estimation its own motion using 
sequential Bayestian inference. SLAM is currently irrelevent for our goal of
providing good estimates of sea ice in a setting which our platform neccessarily 
has good localization through GPS. 
Though developments like MonoSLAM  \cite{MonoSLAM} from Davison et al are able to recover the 3D trajectory of a single camera moving through a scene. 
...
Image mosaicing has been extensively used to gain a greater viewpoint of the world and there exists a large and robust body of work for creating mosaics from digital photographs \cite{Milgram75,  Szeliski95, Schmid97, Shum00, Brown03 } and  
video \cite{Szeliski96, Steedly05}. 
...
An image mosaic can be computed without any knowledge of 
the internal camera calibration parameters such as focal length, 
optical center, or relative camera motion between frames.  We also don't necessarily need to know any information about the UAV's position when the 
images were captured, as this will all be calculated by the mosaicing process, however this metadata can be very useful for improving the mosaic incrementally, and for overcoming regions in which there are few image features. 
...
tranformations \cite{Schmid98, Lowe99, Brown03, Hu06, Elibol08}. 
...
TODO:  know this
Euclidean is used for vector-based descriptors like SURF and SIFT. L2-norm is 
used for descriptors of type float or double, and Hamming distance is used for 
binary descriptors. 
...
This relationship is defined by a homography seen in Equation \ref{eq:homo}, which is a transformation matrix, $H$ that maps points on one image, $i$, to corresponding points on a reference image or mosaic in our case, $i'$. 
The transformations 
...
\cite{SzeliskiBook}.  The most complex transformation considered,
...
images or when there are few distinctive features available \cite{Wang11}. We instead consider only affine tranformations. 
An affine tranformation (defined in Equation \ref{eq:affine} describes scale, rotation, translation, and skew 
...
A robust approach determining the homography is RANSAC or "RANdom SAmple 
...
is used to warp the image onto the reference plane, sometimes called a compositing surface. For our work, which involves a  largely translational camera,
...
This mosaic will include links
...
but because that scene was captured at different times from a different viewpoint, the pixels from the images to be composited at that pixel may be different. Blending involves the process of determining the best color for the pixel in the context of the scene so that the seam cannot be seen. 
...
Skjetne divided ice monitoring into two problems; the first is 
