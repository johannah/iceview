...
However, traveling in ice-laden waters remains dangerous, as current sensing and forecasting capabilities are unable to predict precise movement and characterization of sea ice. 
In this thesis, we present new tools and techniques for making sense of high-resolution aerial photography of sea ice for ice surveillance and monitoring for immediate analysis of data collected with unmanned aerial vehicles (UAVs). We provide a robust method for producing a single overview mosaic with ice concentration summaries from the hundreds of photos captured in a single UAV flight. 
