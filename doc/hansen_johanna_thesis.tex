...
Traditional remote sensing typically referred to spatial data  captured from platforms such as satellites or manned aircraft.  
These platforms generally 
observe the earth from a higher altitude and thus a lower resolution than what we are concerned with here and 
thus have looser requirements for stitching accuracy. For example, a popular instrument 
for observing the earth from satellite is MODIS, the Moderate Resolution Imaging 
...
unmanned aircraft is much more flexible in terms of capture frequency and location. However, potential coverage area is also much more limited with a single sensor when compared to high-altitude systems.  Although our system 
...
automated methods of interpreting the data.  In the Sections \ref{sec:ice} and 
\ref{sec:map} we provide background on two data products that enable faster 
qualification and assesment of the terrain captured by remote sensing. 
...
please refer to a review by Colomina \cite{Colomina14}. For now, we'll focus on 
the sensors and qualities needed for UAVs capable of performing ice coverage missions. 
...
which shaped like a typical airplane, are 
typically more stable and have longer range than rotary wing vehicles, which propel themselves like a helicopter. However, fixed-wing vehicles have 
a more complicated deployment, requiring horizontal space for takeoff and landing. This can be difficult on a ship with limited deck space.

UAVs also vary greatly 
in their computational capabilities and complexity. Many are capable of autonomous flight, while others rely solely on remote control by humans. For 
...
of UAV localization. Although the GPS provides frequent updates for the vehicle position, this calculation contains some ammount of error that is almost always too large to perform image stitching at any reasonable resolution. 
...
The vehicle used in our experiments is a fixed-wing UAV pictured in Figure \ref{phantom}. This vehicle has an endurance of 
