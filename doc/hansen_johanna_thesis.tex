...
Sea ice has a large impact on the global climate, maintaining a heat sink 
in the poles and increasing the Earth's albedo \cite{Sephton94}. It is important to 
study the concentration and distribution of sea ice at a fine scale, because it can hinder travel and provide significant scientific information. We utilize low-cost unmanned aerial vehicles to 
make flexible, high-resolution surveys of sea ice.  
We provide algorithms for mosaicing the sea ice into a single scene as well as 
...
together by hand. In modern times, satellites and digital photography advanced the field 
of remote sensing. To work with the new data, we began to stitch and analyze large volumes of images 
with computers. Image stitching is now commonplace and the technology is embedded 
...
AutoPano \cite{AutoPano}, Hugin \cite{Hugin}, and PhotoScan \cite{Photoscan}. In this work, we 
...
type of work include motion recovery, image retrieval, and robot navigation 
...
to converge, but generally results in a more correct mosaic \cite{Pizarro-thesis03}. 


However, many modern image alignment problems prefer real-time or near real-time 
approaches and there has been recent work in this area exploiting metadata from the camera platform. 
...
to give per-pixel alignment for high-resolution images at close range (<100m). 
...
As introduced in Section \ref{sec:features}, there exist a wide variety of feature extraction and description
...
noise.  However, we found that for many of our images which contained only pictures of ice, very few keypoints could be located in the images (see Figure \ref{orb_scale_bad}).  
...
The results of this experiment are seen in Table \ref{tab:rot} and demonstrated in Figures \ref{scale_bad_features} and \ref{rot_bad_features}. 
...
\section{Thresholding with GLCM}
...
be influenced. From this point on, we perform k-means clustering on the ice portion of the imagery. 
