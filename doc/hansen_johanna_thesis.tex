...
\cite{computermosaic, milgram2, Szeliski97, Szeliski00, marcon2013, pizarro,   
recognising, autostitch, Autopano, PhotoScan }, video \cite{Szeliski-video} and for full 3D 
reconstruction \cite{Szeliski-3D}. These methods are more fully 
reviewed in \cite{criticalreview, Zitova}.
...
slow and memory intensive \cite{Lucas81aniterative, Barnea, Szeliski-direct}.  Feature
...
with less computational cost than the direct method \cite{featbased}. 
...
photo mosaics \cite{ caballero, incremental, cheng, Pritt, Kekec-real-time, mosaicimu, vousdoukas, Wang}.
...
Registration \cite{Barnea, Szeliski-refine}
...
Flaten discussed mosaicing sea ice images for the purpose of UAV navigation in \cite{Norway}, 
...
\cite{Fleischer}. In 2003, Pizarro introduced a zernike-feature based 
approach to mosaic large scale featureless 
scenes underwater with lighting correction \cite{pizarro-thesis, pizarro}. 
Large area addition aided by navigational sensors 
to generate 3D submaps that are merged using bundle adjustment \cite{large-pizarro}.
MATISSE, released 
in 2004 also provides mosaicing capability to seabed videos with navigation data
in conjunction with ArcView\cite{MATISSE-realtime, MATISSE}. 
TODO: read \cite{Eustice, Ferrer, Fleischer, MATISSE, MATISSE-realtime}
In 2013 LAPM was 
introduced to provide a general tool for underwater mosaicing for end-user scientists 
in Matlab \cite{marcon2013}. It uses an open-source implementation of 
SIFT features to match images in addition to track information. 

Small dynamic scene mosaics have been tackled in \cite{Fitzgibbon, Rav-Acha, Skeliski-ghost}. 
...
sea and foam and thresholding based on these intensities \cite{vousdoukas}. Because 
...
data \cite{mosaic-error}.

MEASURE CHANGE \cite{vent-growth}


Prathap gives a thorough review of the current state of mosaicing \cite{criticalreview}. 
...
considered in \ref{sec:features} \cite{SIFT}. Other research has 
produced feature descriptors that have been demonstrated to work under various conditions \cite{Baumberg, dudek, SURF, ORB}. 
Zernike \cite{psuedo_zernike_mosaic, pizarro, dudek, dudekold}
...
Used zernike to create mosaic - read more \cite{psuedo_zernike_mosaic}
Zernike Polynomials 
...
TODO: read \cite{Szeliski-refine}
%%%%%%%%%%%%%%%%%%%%%%
TODO: read \cite{Szeliski-3D} about 3d stuff 
...
Maybe talk about ghosting \cite{Skeliski-ghost}
