...
\title{\textbf{IceView: A System for mosaicing Sea Ice using Zernike Shape Descriptors}}
...
data. TODO: why 
Stitching and warping errors typically come from the accuracy of feature 
...
This is not necessary, but will increase convergence time. 
Most modern approaches register images by selecting key features from each image 
and attempting to match them to other images of the scene. There are many approaches 
for selecting features and describing them 
that will be discussed in \ref{sec:features}. The features are matched between 
images using similarity measurements.  TODO add content

\section{Description of Features}
The algorithm selected for feature or keypoint selector is important for the 
