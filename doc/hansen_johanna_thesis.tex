...
\begin{abstract}
 Polar regions play an outsized role in the future of travel, commerce, and 
defense as sea ice retreats in a changing climate and opens ocean passageways and resources for the first time in modern history. 
However, traveling in ice-laden waters remains dangerous, as current sensing and forecasting capabilities are unable to predict precise movement and characterization of sea ice. Unmanned Aerial Vehicles (UAVs) present the most promising technology to improve a vessel's situational awareness because of their low cost and safety when compared to helicopters, high-frequency and high-resolution when compared to satellite sensing, and wide-coverage when compared to marine radar. 
The prime goal of this thesis is to enable the immediate analysis and utility of high-resolution aerial imagery captured by UAVs.  present new tools and techniques for making sense of high-resolution aerial photography of sea ice for ice surveillance and monitoring for immediate analysis of data collected with unmanned aerial vehicles (UAVs). We provide a robust method for producing a single overview mosaic with ice concentration summaries from the hundreds of photos captured in a single UAV flight. 
Although image mosaicing has become a fairly routine process with modern computers and common scenes, traditional 
image mosaicing algorithms fail when faced with sea ice. 
The featureless nature of solid pack ice and the dynamic nature of 
the sea make it impossible to determine image overlap without our contributions. We provide a method for segmenting ice from the ocean surface to mask dynamic features. We also introduce a Bayesian filtering approach to image mosaicing that integrates sensor information from the UAV to improve image matching. In addition, we produce a provide a new baseline for sea ice segmentation and compare feature descriptors for this dataset.  
...
Polar regions play an outsized role in the future of travel, commerce, and 
defense as the ice retreats and opens ocean passageways for the first time in modern history. 
...
data is captured at a much higher resolution and can be gathered at the
operator's discretion. This means that data for a particular region can be updated whenever 
...
In addition we tackle the problem of determining sea ice concentration,
an important problem when planning routes by sea for surface and underwater vessels.

\section{Thesis Overview}
Currently, there is no practical or robust method of generating an image mosaic 
of high-resolution, large-scale sea ice imagery. We propose to solve this by 
developing a segmentation scheme to mask dynamic features in the imagery and by 
utilizing camera pose information to align images without valid features. 
This thesis demonstrataes large-area mosaicing addressing these issues with an effective 
image registration technique. 
In addition to the mosaic, we also provide a sea ice concentration map and produce 
pixel-wise classification of the scene. 

We demonstrate feature descriptor effectiveness for this class of data with regards 
to robustness against rotation, scaling, and changes in intensity.  
We break the problem down into 3 distinct phases: 
\begin{itemize}
    \item{Pixelwise Classification and Masking of Water}
    \item{Feature Descriptor Performance Evaluation}
    \item{Topology estimation using Bayesian Estimation for Registration}
\end{itemize}

The first phase is related to a long history of sea-ice segmentation that has 
been performed for low-resolution satellite imagery. In Chapter \ref{chap:ice}, 
we evaluate past approaches for ice/water classification used on satellite imagery and extend 
it to work on our high-resolution dataset. 
In the second phase, described in Chapter \ref{chap:feat}, we evaluate several 
feature descriptors and present a method 
for learning feature descriptors for our dataset. 
In our final phase, Chapter \ref{chap:mosaic}, we describe the proccess of mosaicing and evaluate a global, 
image-only based approach against our method of tracking image features with 
sensor information that reveals the camera's pose.
...
\section{High-Resolution Sea Ice Imagery}
UAVs are rapidly becoming the medium of choice for performing sea ice
reconnaissance as indicated several recent publications. 
TODO cite 2013
...
In this paper, Skejetne divided ice monitoring into two problems; the first is 
...
over time. This 
 involves recognizing icebergs on temporally spaced observations and tracking 
their movement using UAVs.  


Modern working ships typically utilize a computer control systems called
\emph{dynamic positioning} (DP) for navigation and station keeping. These 
systems  utilize a variety of sensors to maintain position usually including GPS, 
IMUs, wind sensors, and current sensors like Doppler Velocity Loggers (DVLs) to 
calculate the ship's current position and the necessary actuation of the  
propulsion and steering systems to move to a prescribed location.
Dynamic Positioning is a vital part of modern ship navigation and is especially 
...
for ships working in these water is essential to the proper function of DP systems, but lacking in most systems. 

Lagunov et al. \cite{Lagunov14} describes the value of sea ice monitoring and provided an 
overview of useful sensor platforms including the UAV. This paper 
provides a thorough description 
of the many considerations of sensors and operational constraints involved in 
deploying a UAV for ice observation, but 
did not cover problems related to organizing 
the imagery or automatically evaluating ice coverage.

In \cite{Billington15}, Billington et al. proposed the use of UAVs for shape estimation of icebergs at sea
with UAVs equipped with imaging systems and provided a description of requirements 
for operations. He proposed a neural network to perform 
...
images with several types of ice and water. Zhang then demonstrates segmentation of floes using a Gradient Vector Flow Snake Algorithm with a circular shape that deforms to 
...
This author used Otsu thresholding segmentation on several photos collected from a UAV
and from simulated imagery, though no baseline was provided for evaluation.
...
\subsubsection{Metadata Approach}
...
\section{Relating Overlapping Imager}
